...
You are provided with the NetworkStats of all the peers in the room. The data is stored with last_updated timestamp, with NetworkStats of peerId mapped with the quality of the Network.
...
- `networkStats` - This is the object that stores the network stats of all the peers in the room. The data is stored with last_updated timestamp, with NetworkStats of peerId mapped with the quality of the Network.
